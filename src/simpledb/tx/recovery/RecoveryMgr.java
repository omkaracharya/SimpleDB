...
public class RecoveryMgr {
   ...
   public int setInt(Buffer buff, int offset, /*int oldval,*/ int newval) {
      //Get oldval from the existing value in buffer
   ...
   private void doRecover() {
      Collection<Integer> commitList = new ArrayList<Integer>();
      Collection<Integer> rollbackList = new ArrayList<Integer>();
      ...
      while (iter.hasNext()) {
         ...
         if (rec.op() == CHECKPOINT)
            ...
            commitList.add(rec.txNumber());
         else if (rec.op() == ROLLBACK)
            rollbackList.add(rec.txNumber());
         else if (!commitList.contains(rec.txNumber()) && !rollbackList.contains(rec.txNumber()))
            rec.undo(txnum);
      }
      ...
      while (iter.hasNextForward()) {
         ...
         //if it is an update record and exists in commitlist, redo
         if ((rec.op() == SETINT || rec.op() == SETSTRING) && commitList.contains(rec.txNumber())) {
            rec.redo(txnum);
         }
      }
   }
