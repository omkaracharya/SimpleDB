...
import static simpledb.log.LogIterator.PTR_SIZE;
...
public class LogMgr implements Iterable<BasicLogRecord> {
    ...
    public LogMgr(String logfile) {
        ...
        if (logsize == 0)
            ...
            //set current pos at end of last record
            currentpos = getLastRecordPosition() + PTR_SIZE;
        }
    }
    ...
    public synchronized int append(Object[] rec) {
        int recsize = 2 * INT_SIZE;  // 8 bytes for 2 integers that points to the previous and next log record
        for (Object obj : rec)
            recsize += size(obj);
    ...
    private void appendNewBlock() {
        //add last_pos value to first 4 bytes of block
        setLastRecordPosition(0);
        currentpos = INT_SIZE;

        //add next record pointer to next 4 bytes
        mypage.setInt(currentpos, 0);
        currentpos += INT_SIZE;
    ...
    private void finalizeRecord() {
        //set next record pointer of the previous record to the current position
        mypage.setInt(getLastRecordPosition() + INT_SIZE, currentpos);
        //set previous record pointer of current record to the previous record's position
        mypage.setInt(currentpos, getLastRecordPosition());
        //set last_pos value at start of the block to current position
        setLastRecordPosition(currentpos);
        currentpos += INT_SIZE;

        //set default next record pointer of the current(last) record to 0
        mypage.setInt(currentpos, 0);
        currentpos += INT_SIZE;
    }
